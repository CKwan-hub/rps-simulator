...
function App() {
  ...
  useEffect(() => {
    ...
    if (simulationStarted && !paused) {
      interval = setInterval(moveIcons, 20); // Adjust speed as needed (20 milliseconds)
    }

    return () => clearInterval(interval);
  }, [simulationStarted, paused]);
  ...
  return (
    <div className="App">
      {!simulationStarted && <button className='start-btn' onClick={startSimulation}>Start Simulation</button>}
      {/* <InfoBar
        showInfoBar={simulationStarted}
        onPauseClick={pauseSimulation}
      ...
      />
      <Grid icons={icons} simulationStarted={simulationStarted} paused={paused} />
    </div>
  );
}
